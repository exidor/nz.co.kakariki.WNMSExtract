...
/**
 * NPO48 was a non production test box with a DHCP IP. 48 may not necessarily match the IP address
 * specified in its definition. This is a JDC Connection implementation for that box
 * @author jnramsay
 *
 */
public class NPO48JDCConnector implements ALUJDCConnector{

	private static Logger jlog = Logger.getLogger("com.alcatel_lucent.nz.wnmsextract.database.NPO48JDCConnector");
	...
	private static final String DEF_HOST = "139.188.126.204";
	...
	*/
	...
	public NPO48JDCConnector(ALUConnectionDefinitions cd){
		setConnectionDefinitions(cd);
		init();
	}

	public void init(){
		jlog.info("Setup connection NPOJDC :: "+url());
		...
		jlog.info("Connection complete, NPOJDC :: "+url());
	}
