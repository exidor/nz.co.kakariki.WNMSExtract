...
/**
 * Board table inserter. Child of NodeBEquipment has no data of its own as acts as
 * an intermediate step to its child tables, Cem and Ccm
 * @author jnramsay
 */
public class ALUBoardTable {
	...
	public static void insertData(ALUJDCConnector ajc, XmlObject xmlobject, String nbid, Timestamp ts) {
		...
		for (BoardType bt : nbet.getBoardArray()) {
			if (bt.isSetCcm()) {
				ALUCcmTable.insertData(ajc, bt, nbid, ts, nbeid);
			} else {
				ALUCemTable.insertData(ajc, bt, nbid, ts, nbeid);
			}
		}
	}
