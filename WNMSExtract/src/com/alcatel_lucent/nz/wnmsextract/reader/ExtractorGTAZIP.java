...
@Deprecated
public class ExtractorGTAZIP extends Extractor {

	private static Logger jlog = Logger.getLogger("com.alcatel_lucent.nz.wnmsextract.schedule.ExtractorGTAZIP");

	public ExtractorGTAZIP(){
	}

	/** process docs*/
	public void processDocType(DatabaseType dbname, DocumentType doctype, boolean atflag){

		String datapath = "G:\\data\\gta\\data\\source\\";
		String temppath = "G:\\data\\gta\\data\\temp\\";

		jlog.info("Extraction Job. "+datapath+", "+dbname+", "+doctype);
		...
		for(File file : processDocuments(datapath,temppath,doctype))
		{
	...
	private List<File> processDocuments(String datapath,String temppath,DocumentType doctype){
		...
		try {
			synchronized(fs){
				...
				fs.setCalendar(cal);
				fs.setSourcePath(new File(datapath));
				fs.setTempPath(new File(temppath));
				fs.extract();
	...
	@Override
	public String toString(){return "GTA Extractor ";}//+cal;}
