-- create type rnc_bh_container as (id varchar(8), ts timestamp, val double precision);
create or replace function rnc_bh(timestamp,int,varchar) returns setof rnc_bh_container as $$  
 select 
	d1.id,
	d1.ts,
	d1.val
